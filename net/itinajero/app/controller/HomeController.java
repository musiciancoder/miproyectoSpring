...
(cont) sin embargo Spring creo una instancia de nuestra clase de servicio de forma automatica y la inyecto en nuestro controlador con la anotacion @autowired en nuestra variable servicePeliculas*/
	...
	public String mostrarDetalle(Model model, @RequestParam("idMovie") int idPelicula, @RequestParam("fecha") String fecha) { //idMovie y fecha en azul vienen del home.jsp en la linea href="detail?idMovie=${pelicula.id }&fecha=${fechaBusqueda}
		...
		model.addAttribute("pelicula", servicePeliculas.buscarPorId(idPelicula));// Aca se pasa al modelo (idPelicula);  servicePeliculas.buscarPorId(idPelicula) funciona por inyeccion de dependencias; buscarPorId es metodo de clase de servico PeliculasServiceJPA	
